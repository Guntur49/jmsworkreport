...
<head>
    ...
    <title>Payment Jurnal - Portal Kerja Sama</title>
    <style>
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="content-body">
            <div class="breadcrumb">Sub Menu / {{ $selectedHospital === 'all' ? 'Semua Rumah Sakit' : session('selected_hospital') }} / Payment Jurnal</div>
            <div class="page-header">
                <h1>Payment Jurnal</h1>
            </div>

            <div class="card">
                <div class="card-title">Filter Riwayat Pembayaran</div>
                <form method="GET" action="{{ route('payment-jurnal.index') }}">
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            <label>Tanggal Realisasi Mulai</label>
            ...
            <div class="summary-row">
                <div class="summary-card summary-card-primary">
                    ...
                    <span>{{ $paymentHistory->count() }}</span>
                </div>
                <div class="summary-card">
                    <strong>Total Nominal</strong>
                    <span>Rp {{ number_format($paymentHistory->sum('nominal'), 0, ',', '.') }}</span>
                </div>
                <div class="summary-card">
                    <strong>Total Net Payment</strong>
                    <span>Rp {{ number_format($paymentHistory->sum('net_payment'), 0, ',', '.') }}</span>
                </div>
            </div>

            <div class="card">
                <div class="card-title">Daftar Pembayaran Terbayar</div>
                @if($paymentHistory->isEmpty())
                    <div class="empty-state">
                        Belum ada riwayat pembayaran terbayar. Periksa kembali filter tanggal atau rumah sakit terpilih.
                    </div>
                @else
                    <table class="data-table">
                        ...
                        <tbody>
                            @foreach($paymentHistory as $index => $row)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ $row->tanggal_realisasi ? \Carbon\Carbon::parse($row->tanggal_realisasi)->format('d M Y') : '-' }}</td>
                                    <td>{{ $row->tahap }}</td>
                                    <td>{{ $row->nama_rs ?? '-' }}</td>
                                    <td>{{ $row->penerima_alokasi }}</td>
                                    <td>{{ $row->deskripsi }}</td>
                                    <td>Rp {{ number_format($row->nominal, 0, ',', '.') }}</td>
                                    <td>Rp {{ number_format($row->net_payment, 0, ',', '.') }}</td>
                                    <td>
